...
$url = "https://trendapi.moonclubs.in/30api.php?api_key=" . $apiKey;

$query = "SELECT manual_number FROM gelluonduhogu_zehn_zehn WHERE id = 1";
...
$url = 'https://jdb.delhigame.club/win30.php';

// Fetch the latest number
$latestNumber = @file_get_contents($url);

if ($latestNumber === false || trim($latestNumber) === '') {
    // If fetching fails or response is empty, assign a random number between 0 and 9
    $kadimesucyanka = $defaultNumber;
} else {
    // Use the fetched number
    $kadimesucyanka = $defaultNumber;
}
