<?php 
    ...
    $samasye = "SELECT parichaya, kalaparichaya, ojana, menge, wettanzahl, ketebida, phalaphala, sesabida, ergebnis, zufallig, tiarikala, 2 AS game_id
                FROM bajikattuttate_drei
                WHERE byabaharkarta = $shonuid
                UNION ALL
                SELECT parichaya, kalaparichaya, ojana, menge, wettanzahl, ketebida, phalaphala, sesabida, ergebnis, zufallig, tiarikala, 3 AS game_id
...
else if ($gameType == '-1' && $type == 2) {
  $samasye = "SELECT id, orderNo, vendorCode, gameName, betAmount, validBetAmount, winAmount, winLossAmount, betTime
  FROM xd_apigame_bets
  WHERE uid = $shonuid AND subCategory = 'Slot'
  ORDER BY id DESC
  LIMIT $pageSize OFFSET $samatolana";
...
else if ($gameType == 'JILI' && $type == 2) {
  $samasye = "SELECT id, orderNo, vendorCode, gameName, betAmount, validBetAmount, winAmount, winLossAmount, betTime
  FROM xd_apigame_bets
  WHERE uid = $shonuid AND subCategory = 'Slot' AND vendorCode = 'JILI'
  ORDER BY id DESC
  LIMIT $pageSize OFFSET $samatolana";
...
else if ($gameType == '-1' && $type == 3) {
  $samasye = "SELECT id, orderNo, vendorCode, gameName, betAmount, validBetAmount, winAmount, winLossAmount, betTime
  FROM xd_apigame_bets
  WHERE uid = $shonuid AND subCategory = 'Fishing'
  ORDER BY id DESC
  LIMIT $pageSize OFFSET $samatolana";
...
else if ($gameType == 'JILI' && $type == 3) {
  $samasye = "SELECT id, orderNo, vendorCode, gameName, betAmount, validBetAmount, winAmount, winLossAmount, betTime
  FROM xd_apigame_bets
  WHERE uid = $shonuid AND subCategory = 'Fishing' AND vendorCode = 'JILI'
  ORDER BY id DESC
  LIMIT $pageSize OFFSET $samatolana";
...
else if ($gameType == '-1' && $type == 6) {
  $samasye = "SELECT id, orderNo, vendorCode, gameName, betAmount, validBetAmount, winAmount, winLossAmount, betTime
  FROM xd_apigame_bets
  WHERE uid = $shonuid AND subCategory IN ('Casino', 'Lobby', 'Poker')
  ORDER BY id DESC
  LIMIT $pageSize OFFSET $samatolana";
...
else if ($gameType == 'JILI' && $type == 6) {
  $samasye = "SELECT id, orderNo, vendorCode, gameName, betAmount, validBetAmount, winAmount, winLossAmount, betTime
  FROM xd_apigame_bets
  WHERE uid = $shonuid AND subCategory IN ('Casino', 'Lobby', 'Poker') AND vendorCode = 'JILI'
  ORDER BY id DESC
  LIMIT $pageSize OFFSET $samatolana";
...
$samasyephalitansa_sankhye = $samasyephalitansa_ondu->fetch_assoc()['total_count'];
  ...
  $samasye = "SELECT id, orderNo, vendorCode, gameName, betAmount, validBetAmount, winAmount, winLossAmount, betTime
              FROM xd_apigame_bets
              WHERE uid = $shonuid AND subCategory = 'Slot'
                AND DATE(betTime) >= DATE('$startDate') 
                AND DATE(betTime) <= DATE('$endDate')
              ORDER BY id DESC
              LIMIT $pageSize OFFSET $samatolana";
$samasyephalitansa = $conn->query($samasye);
...
$samasyephalitansa_sankhye = mysqli_num_rows($samasyephalitansa_ondu);
              ...
              while ($row = $samasyephalitansa->fetch_assoc()) {
                  ...
                  if ($gameType == '-1' && $type == 2) {
                      $data['list'][$i]['orderNo'] = $row['orderNo'];
                      $data['list'][$i]['venderCode'] = $row['vendorCode'];
                      $data['list'][$i]['gameName'] = $row['gameName'];
                      $data['list'][$i]['betTime'] = $row['betTime'];
                      $data['list'][$i]['betAmount'] = round((float)$row['betAmount'], 2);
                      $data['list'][$i]['validBetAmount'] = round((float)$row['validBetAmount'], 2);
                      $data['list'][$i]['winAmount'] = round((float)$row['winAmount'], 2);
                      $data['list'][$i]['winLossAmount'] = round((float)$row['winLossAmount'], 2);
                      $data['list'][$i]['waterAmount'] = 0.0;
                      $data['list'][$i]['serviceFeeAmount'] = 0.0;
                      $data['list'][$i]['orderType'] = 0;
                      $data['list'][$i]['orderStatus'] = 1;
                  } 
