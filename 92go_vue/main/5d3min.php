...
<?php
	...
	$samasye = "SELECT atadaaidi FROM `gelluonduhogu_aidudi_drei` ORDER BY kramasankhye DESC LIMIT 1";
	$samasyephalitansa = $conn->query($samasye);
	$samasyephalitansa_dhadi = mysqli_fetch_assoc($samasyephalitansa);
	
	$munde = mysqli_query($conn,"SELECT sankhye FROM `hastacalita_phalitansa_aidudi_drei` WHERE `sthiti`='1'");
...
<body>
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-sm-12 mb-4 mb-xl-0">
              <h4 class="font-weight-bold text-dark">5D 3 Min</h4>
            </div>
          </div> 
		  <div class="row">
		  ...
		  <div class="row">
            <div class="col-sm-12">
			  ...
			  <form action="itticina_geluvu_aidudi_drei" id="pre" method="POST">
  ...
  <script>	
	...
	function wingoonetotal()
	{
		$.ajax({
		type: "Post",
		url: "ottu_gellaluhogiondu_aidudi_drei.php",
	...
	var table = $('#example2').DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "detavannunirvahisi_aidudi_drei.php",
		"paging": true,
		"lengthChange": false,
		"searching": true,
		"ordering": false,
	...
	function resetman(){		
		$.ajax({
			type: "Post",
			data:"stat=" + "1",
			url: "maruhondisi_gellalu_aidudi_drei.php",
			success: function (html) {
			 console.log(html);
			  return false;
			  },
