...
<?php
	...
	$samasye = "SELECT atadaaidi FROM `gelluonduhogu_kemuru_zehn` ORDER BY kramasankhye DESC LIMIT 1";
	$samasyephalitansa = $conn->query($samasye);
	$samasyephalitansa_dhadi = mysqli_fetch_assoc($samasyephalitansa);
	
	$munde = mysqli_query($conn,"SELECT sankhye FROM `hastacalita_phalitansa_kemeru_zehn` WHERE `sthiti`='1'");
...
<body>
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-sm-12 mb-4 mb-xl-0">
              <h4 class="font-weight-bold text-dark">K3 10 Min</h4>
            </div>
          </div> 
		  <div class="row">
		  ...
		  <div class="row">
            <div class="col-sm-12">
			  ...
			  <form action="itticina_geluvu_kemuru_zehn" id="pre" method="POST">
				<h6 style="text-align:center; font-weight:bold">Prediction Form</h6>
				<div class="d-flex align-items-center">				
					<input type="text" name="username" id="next" placeholder="Enter a number from 111-666" class="flex-grow-1 cool-input" style="height: 40px;">					
				</div>
		  ...
		  <div class="row">
			<div class="col-sm-12 offset-sm-4 mt-3">
				<h5 id="copied" style="text-align:center; font-weight:bold">Enter a number from 111-666</h5>
			</div>
		  </div>
        </div>
        <footer class="footer">
  ...
  <script>	
	...
	function wingoonetotal()
	{
		$.ajax({
		type: "Post",
		url: "ottu_gellaluhogiondu_kemuru_zehn.php",
	...
	var table = $('#example2').DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "detavannunirvahisi_kemuru_zehn.php",
		"paging": true,
		"lengthChange": false,
		"searching": true,
		"ordering": false,
	...
	function resetman(){		
		$.ajax({
			type: "Post",
			data:"stat=" + "1",
			url: "maruhondisi_gellalu_kemuru_zehn.php",
			success: function (html) {
			 console.log(html);
			  return false;
			  },
	...
	function startTimer(serverTime) {
		...
		var interval = setInterval(function () {			
			...
			var sec2 = seconds % 10;
			if(distance == 600){
				document.getElementById("demo").innerHTML = "<span class='timer'>"+Math.floor(minutes)+"</span>" + "<span>:</span>" +"<span class='timer'>"+seconds+"</span>";	
			}
			else{
