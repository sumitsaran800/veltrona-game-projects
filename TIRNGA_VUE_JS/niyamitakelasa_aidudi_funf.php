...
$bartamankalakrama = $currentDate . "10203" . $uniqueSequence;
$bartamankalakrama = $bartamankalakrama + 1;

$prathama = $bartamankalakrama; 
$sesa = $currentDate . "10203" . sprintf("%04d", ceil(86400 / 300));
	
	$tarika = date('Y-m-d H:i:s');
	
	$dekhakalakrama = mysqli_query($conn,"select atadaaidi from `gelluonduhogu_aidudi_funfu` order by kramasankhye desc limit 1");
