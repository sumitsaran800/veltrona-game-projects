			...
			if($shonusign == $signature){
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
						...
						$existance = mysqli_query($conn,"SELECT `daysonearth`, `totalblessings`, `amen` FROM `cihne` WHERE `identity`='".$shonuid."' ORDER BY `dearlord` ASC");
						$i = 0;
						while($row = mysqli_fetch_array($existance)){							
							$data['list'][$i]['continuousDayContinue'] = $row['daysonearth'];
							$data['list'][$i]['markDayTime'] = $row['amen'];
							$data['list'][$i]['amount'] = $row['totalblessings'];
							$i++;
						}
