	...
	$table = <<<EOT
	 (
		SELECT
		  ...
		  bajikattuttate_funf.ojana,
		  bajikattuttate_funf.ketebida,
		  (SELECT mobile FROM  shonu_subjects WHERE id = bajikattuttate_funf.byabaharkarta) AS mobile,
		  (SELECT motta FROM  shonu_kaichila WHERE balakedara = bajikattuttate_funf.byabaharkarta) AS balance
		FROM bajikattuttate_funf
		WHERE bajikattuttate_funf.kalaparichaya = (SELECT atadaaidi FROM gelluonduhogu_funf ORDER BY gelluonduhogu_funf.kramasankhye DESC LIMIT 1)
	 ) temp
	EOT;	
