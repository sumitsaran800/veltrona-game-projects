...
include("conn.php");
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST"){
		...
		if(empty($err))
		{
			...
			$sqla = mysqli_query($conn,"UPDATE hastacalita_phalitansa_aidudi_funf SET sthiti='0'");
			$sql = "UPDATE hastacalita_phalitansa_aidudi_funf SET sthiti='1', sankhye='".$username."' WHERE shonu='1'";

			$conn->query($sql);
			if ($conn->query($sql) === TRUE) {
				header("Location: 5d5min.php");
			} else {
